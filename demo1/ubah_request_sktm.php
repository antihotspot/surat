...
if(isset($_POST['ubah'])){
	...
	if (!empty($_FILES['ktp']['tmp_name'])) {
        $file_ktp = $_POST['nik']."_ktp_".$timestamp.".jpg";
    } else {
        echo "<script language='javascript'>swal('Gagal...', 'Ubah Gagal: File KTP tidak boleh kosong', 'error');</script>" ;
    }

    // Validasi file KK
    if (!empty($_FILES['kk']['tmp_name'])) {
        $file_kk = $_POST['nik']."_kk_".$timestamp.".jpg";
    } else {
        echo "<script language='javascript'>swal('Gagal...', 'Ubah Gagal: File KK tidak boleh kosong', 'error');</script>" ;
    }
